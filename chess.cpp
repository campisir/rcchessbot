...
Chess::Chess()
{
	...
	bool setupfail = true;
	std::fstream newfile;
	std::cout << "\nWelcome to Ryan's chess bot!\n\nEnter starting text file, or enter anything else to start" << std::endl;
	std::cout << "File: ";
	std::cin >> startpos;
	newfile.open(startpos);
	if(newfile.is_open())
	{
		std::cout << "\nNotes:\n";
		setupfail = false;
		while(getline(newfile, read))
		{
			if(read.length() != 6)
			{	
				std::cout << "The read was stopped at line: '" << read << "' as it was not understood" << std::endl;
				break;
			}
			else
			{
				if(read == "turn 1" || read == "turn 0")
				{
					if(read == "turn 1")
					{
						playermove = true;
				...
				for(int i = 0; i < 8; i++)
				{
					for(int j = 0; j < 8; j++)
					{
						if(board[i][j].SquareName() == read.substr(2, 2))
						{
							board[i][j].setPiece(read.at(0), (read.at(5) - '0'));
						}
					}
				}
			}
		...
		for(int i = 0; i < 8; i++)
		{
			for(int j = 0; j < 8; j++)
			{
				...
				{
					kingcount++;
					kingtypes+=board[i][j].getColor();
				}
			}
		}
		if(kingcount < 2)
		{
			std::cout << "This is not a valid position because there are not enough kings!" << std::endl;
			setupfail = true;
		}
		else if(kingcount > 2)
		{
			std::cout << "This is not a valid position because there are too many kings!" << std::endl;
			setupfail = true;
		}
		else if(kingtypes > 1)
		{
			std::cout << "This is not a valid position. Where is white's king??" << std::endl;
			setupfail = true;
		}
		else if(kingtypes < 1)
		{
			std::cout << "This is not a valid position. Where is black's king??" << std::endl;
			setupfail = true;
		}
		DefenseReset();
		if(blackcheck || check)
		{
			std::cout << "Your setup has a king in check!! Backup a move at least" << std::endl;
			setupfail = true;
		}
	...
	{
		playermove = false;
		std::cout << "\nSetting a default board and starting. I play white!\n" << std::endl;
		SetBoard();
	}
	else
	{
		std::cout << "This position is valid! Starting game.. \n" << std::endl;
	}
	for(int i = 0; i < 8; i++)
	{
		for(int j = 0; j < 8; j++)
		{
			//std::cout << "element [" << i << "][" << j << "] is " << board[i][j].SquareName() << " with a " ;
			...
			//std::cout<< board[i][j].getPiece() << " on it" << std::endl;
		}
	}
	std::string in = "go";
	int delay = 0;
	...
	char helper = ' ';
	char helper2 = ' ';
	while(in != "quit" && in != "stop")
	{
		if(in != "board" && !playermove && in != "stats" && in != "resign" && in!= "help")
		{
			PromotionZone();
			CheckForDraws();
			if(enpassant && delay == 1)
                        {
                                delay = 0;
                                enpassant = false;
                        }
			...
			if(forcemove != "")
			{
				for(int i = 0; i < 8; i++)
				{
					for(int j = 0; j < 8; j++)
					{
						if(board[i][j].SquareName() == forcemove.substr(forcemove.length()-2))
						{
							if(forcemove.length() == 2 || forcemove.at(1) == 'x')
							{
								helper = 'p';
							}
							else
							{
								helper = forcemove.at(0);
							}
							if(forcemove.length() == 4 && forcemove.at(1) == 'x')
							{
								helper2 = forcemove.at(0);
							}
							else if(forcemove.length() == 4)
							{
								helper2 = forcemove.at(1);
							}
							if(!ValidMove(board[i][j].getRowIndex(), board[i][j].getColumnIndex(), helper, 0, helper2, true))
							{
								std::cout << "Your move was valid, but I cannot play " << forcemove.at(0) << " to " << forcemove.substr(forcemove.length()-2) << ". I will make my own move" << std::endl;
								WhitetoMove();
							}
							else
							{
								std::cout << "The moves have been made! Your turn again" << std::endl;
							}
						}
					}
				}
			...
			else if(!repetition && !fiftymoverule && !lackmaterial)
			{
				WhitetoMove();
			}
			PromotionZone();
			...
			if(checkmate)
			{
				std::cout << std::endl << "You win by checkmate!" << std::endl << std::endl;
				log.at(log.size()-1).setCheckmate(true);
				break;
			}
			else if(stalemate || (DidIWin() && !blackcheck))
			{
				std::cout << std::endl << "It's a draw by stalemate!" << std::endl << std::endl;
				break;
			}
			else if(DidIWin())
			{
				std::cout << std::endl << "I win by checkmate!" << std::endl << std::endl;
				log.at(log.size()-1).setCheckmate(true);
				break;
			}
			else if(fiftymoverule)
			{
				std::cout << "It's a draw by the fify move rule!" << std::endl << std::endl;
				break;
			}
			else if(repetition)
			{
				std::cout << "It's a draw by repetition!" << std::endl << std::endl;
				break;
			}
			else if(lackmaterial)
			{
				std::cout << "It's a draw by insufficient material!" << std::endl << std::endl;
				break;
			}
			DefenseReset();
			if(blackcheck)
		...
		if(in == "resign")
		{
			std::cout << std::endl << "I win by resignation!" << std::endl << std::endl;
			break;
		}
		if(in == "help")
		{
	...
	std::cout << "Ending board position: " << std::endl;
	PrintBoard();
...
void Chess::PrintMoves()
{
	...
	for(Move m : log)
	{
		...
		if(m.getPiece() == 'p')
		{
			if(m.getCapture() != ' ')
			{
				std::cout << m.getStart().getFile() << "x" << m.getEnd().SquareName();
			}
			else
			{
				std::cout << m.getEnd().SquareName();
			}
			if(m.getPromotion())
			{
				std::cout << "=" << char(toupper(m.getPromotionPiece()));
			}
		}
		...
		{
	        	std::cout << char(toupper(m.getPiece()));
		        if(m.getCapture() != ' ')
			{
				std::cout << "x";
			}	
...
{
	std::string in;
	std::cout << std::endl << "[1] How do I input my moves?\n[2] How do I win?\n[3] How do I see the board?\n[4] The computer made a mistake\n[5] Show commands\n";
	std::cin >> in;
	if(in == "1")
	{
		std::cout << "Pawn is denoted as 'p', knight as 'n', queen as 'q', rook as 'r', bishop as 'b', and king as 'k'\nRows are defined 1 through 8, and columns are defined a through h\n";
		std::cout << "To move a piece, type the first letter of the piece and the square.\nEx. Bishop to c5 ('bc5' is entered)\nDO NOT PUT 'p' IN FRONT OF PAWN MOVES\nEx. Pawn to e5 ('e5' is entered)\n";
		std::cout << "To capture with a pawn, type the column the pawn is on, then 'x', then the landing square.\nEx. The c-pawn captures d4 ('cxd4' is entered)\nDO NOT ADD 'x' WHEN CAPTURING WITH PIECES THAT AREN'T PAWNS\n";
		std::cout << "Ex. Bishop takes c5 ('bc5' is entered)\nIf you have two identical pieces that can move to the same square, specify which one with the column or row (whichever is different)\n";
		std::cout << "Ex. Both Knights can land on e4, one knight is on the g column, one is on the c column. You can move the g knight to e4 ('nge4' is entered)\nTo castle king's side, enter '0-0'\n";
		std::cout << "To castle queen's side, enter '0-0-0'" << std::endl;
	}
	else if (in == "2")
	{
		std::cout << "It's just chess rules, look it up " << std::endl;
	}
	else if (in == "3")
	{
		std::cout << "Enter 'board' to see the board.\n";
	}
	else if(in == "4")
	{
		std::cout << "Sorry bout' that" << std::endl;
	}
	else if(in == "5")
	{
		std::cout << "Enter 'undo' to undo your previous move\nEnter 'scores' to see how the bot scored each of its moves\n";
		...
		std::cout << " the computer bragging that it won\nEnter 'help' for the help menu\n";
	}
	else
	{
...
void Chess::UndoMove()
{
	if(log.size() < 2)
	{
		std::cout << "You have not made a move yet" << std::endl;
		return;
	}
	for(int i = 0; i < 2; i++)
	{
...
bool Chess::BlacksMove(std::string move)
{
	...
	for(int i = 0; i < 8; i++)
	{
		for(int j = 0; j < 8; j++)
		{
			if(board[i][j].SquareName() == square && board[i][j].getColor() != 1)
			{
				...
				else
				{
					if(blackcheck)
					{
						...
						{
							std::cout << "That move would put you in check!" << std::endl;
						}
					}
					else
						std::cout << "That's an illegal move!" << std::endl;
					return false;
			...
			{
				std::cout << "You have a piece on " << square << "!" << std::endl;
				return false;
			}
		}

	}
	std::cout << piece << " to " << square << "? I don't understand that move" << std::endl;
	return false;
...
void Chess::PromotionZone()
{
	...
	for(int i = 0; i < 8; i++)
	{
		...
		{
			log.at(log.size()-1).setPromotion(true);
			std::cout << "I promote to a queen!" <<std::endl;
		...
		for(unsigned int i = 0; i < moveable.size(); i++)
		{
			if(moveable.at(i).getPiece() == 'p')
			{
				if(moveable.at(i).getRowIndex() + 1 < 8)
				{
					if(board[moveable.at(i).getRowIndex()+1][moveable.at(i).getColumnIndex()].IsEmpty())
					{
						if(!moved &&  ValidMove(moveable.at(i).getRowIndex()+1, moveable.at(i).getColumnIndex(), 'p', 0, ' ', false))
						{
							...
							{
								std::cout<< "Pawn to " << board[moveable.at(i).getRowIndex()+1][moveable.at(i).getColumnIndex()].SquareName()<<"!"<<std::endl;
								moved = true;
				...
				if(moveable.at(i).getRowIndex() + 2 < 8 && moveable.at(i).getRowIndex() == 1 && board[moveable.at(i).getRowIndex()+1][moveable.at(i).getColumnIndex()].IsEmpty())
				{
					if(board[moveable.at(i).getRowIndex()+2][moveable.at(i).getColumnIndex()].IsEmpty())
					{
				 		if(!moved && ValidMove(moveable.at(i).getRowIndex()+2, moveable.at(i).getColumnIndex(), 'p', 0, ' ', false))
							...
							if(makemove && decision == movecounter)
							{
                                                		std::cout<< "Pawn to " << board[moveable.at(i).getRowIndex()+2][moveable.at(i).getColumnIndex()].SquareName()<<"!"<<std::endl;
                                                		moved = true;
				...
				if(moveable.at(i).getRowIndex() + 1 < 8 && moveable.at(i).getColumnIndex() + 1 < 8)
				{
					if(board[moveable.at(i).getRowIndex()+1][moveable.at(i).getColumnIndex()+1].getColor() == 1)
					{
						if(!moved && ValidMove(moveable.at(i).getRowIndex()+1, moveable.at(i).getColumnIndex()+1, 'p', 0, ' ', false))
						{
							if(makemove && decision == movecounter)
							{
								std::cout << "Pawn takes " << board[moveable.at(i).getRowIndex()+1][moveable.at(i).getColumnIndex()+1].SquareName() << "!" << std::endl;
								moved = true;
				...
				if(enpassant && (moveable.at(i).getColumnIndex() == enpassantcol - 1 || moveable.at(i).getColumnIndex() == enpassantcol + 1) && moveable.at(i).getRowIndex() == 4 && board[5][enpassantcol].IsEmpty())
				{
					if(!moved && ValidMove(5, enpassantcol, 'p', 0, ' ', false))
					{
						if(makemove && decision == movecounter)
						{
							std::cout << "Pawn takes " << board[5][enpassantcol].SquareName() << " becuase en passant is forced!" << std::endl;
							moved = true;
			...
			if(moveable.at(i).getPiece() == 'n')
			{
				if(moveable.at(i).getRowIndex()+2 < 8 && moveable.at(i).getColumnIndex()+1 < 8)
				{
					if(board[moveable.at(i).getRowIndex()+2][moveable.at(i).getColumnIndex()+1].IsEmpty())
					{
						if(!moved && ValidMove(moveable.at(i).getRowIndex()+2, moveable.at(i).getColumnIndex()+1, 'n', 0, ' ', false))
							...
							if(makemove && decision == movecounter)
							{
                                                		std::cout<< "Knight to " << board[moveable.at(i).getRowIndex()+2][moveable.at(i).getColumnIndex()+1].SquareName()<<"!"<<std::endl;
                                                		moved = true;
					...
					else if(board[moveable.at(i).getRowIndex()+2][moveable.at(i).getColumnIndex()+1].getColor() == 1)
					{
						if(!moved && ValidMove(moveable.at(i).getRowIndex()+2, moveable.at(i).getColumnIndex()+1, 'n', 0, ' ', false))
						{
							if(makemove && decision == movecounter)
							{
								std::cout << "Knight takes " << board[moveable.at(i).getRowIndex()+2][moveable.at(i).getColumnIndex()+1].SquareName() << "!" << std::endl;
								moved = true;
								ValidMove(moveable.at(i).getRowIndex()+2, moveable.at(i).getColumnIndex()+1, 'n', 0, ' ', true);
							}
							else if(makemove)
				...
				if(moveable.at(i).getRowIndex()+1 < 8 && moveable.at(i).getColumnIndex()+2 < 8)
							...
							if(makemove && decision == movecounter)
							{
                                                        	std::cout<< "Knight to " << board[moveable.at(i).getRowIndex()+1][moveable.at(i).getColumnIndex()+2].SquareName()<<"!"<<std::endl;
                                                        	moved = true;
							...
							if(makemove && decision == movecounter)
							{
                                                        	std::cout << "Knight takes " << board[moveable.at(i).getRowIndex()+1][moveable.at(i).getColumnIndex()+2].SquareName() << "!" << std::endl;
                                                        	moved = true;
								ValidMove(moveable.at(i).getRowIndex()+1, moveable.at(i).getColumnIndex()+2, 'n', 0, ' ', true);
							}
							else if(makemove)
			...
			if(moveable.at(i).getPiece() == 'q')
			{
				for(int x = 1; x <= (7 - moveable.at(i).getRowIndex()); x++)
				{
					if(board[moveable.at(i).getRowIndex()+x][moveable.at(i).getColumnIndex()].IsEmpty())
					{
						if(!moved && ValidMove(moveable.at(i).getRowIndex()+x, moveable.at(i).getColumnIndex(), 'q', 0, ' ', false))
						{
							if(makemove && decision == movecounter)
							{
								std::cout << "Queen to " << board[moveable.at(i).getRowIndex()+x][moveable.at(i).getColumnIndex()].SquareName() << "!"  <<std::endl;
								moved = true;
					...
					else
					{
						if(board[moveable.at(i).getRowIndex()+x][moveable.at(i).getColumnIndex()].getColor() == 1)
								...
								if(makemove && decision == movecounter)
								{
                                                        		std::cout << "Queen takes " << board[moveable.at(i).getRowIndex()+x][moveable.at(i).getColumnIndex()].SquareName() << "!" << std::endl;
                                                        		moved = true;
									ValidMove(moveable.at(i).getRowIndex()+x, moveable.at(i).getColumnIndex(), 'q', 0, ' ', true);
								}
								else if(makemove)
				...
				for(int x = 1; x <= (7 - moveable.at(i).getColumnIndex()); x++)
				{
					if(board[moveable.at(i).getRowIndex()][moveable.at(i).getColumnIndex()+x].IsEmpty())
					{
						if(!moved  && ValidMove(moveable.at(i).getRowIndex(), moveable.at(i).getColumnIndex()+x, 'q', 0, ' ', false))
						{
							if(makemove && decision == movecounter)
							{
								std::cout << "Queen to " << board[moveable.at(i).getRowIndex()][moveable.at(i).getColumnIndex()+x].SquareName() << "!" << std::endl;
								moved = true;
					...
					else
					{
						if(board[moveable.at(i).getRowIndex()][moveable.at(i).getColumnIndex()+x].getColor() == 1)
								...
								if(makemove && decision == movecounter)
								{
                                                                	std::cout << "Queen takes " << board[moveable.at(i).getRowIndex()][moveable.at(i).getColumnIndex()+x].SquareName() << "!" << std::endl;
                                                                	moved = true;
									ValidMove(moveable.at(i).getRowIndex(), moveable.at(i).getColumnIndex()+x, 'q', 0, ' ', true);
								}
								else if(makemove)
				...
				for(int x = moveable.at(i).getRowIndex() - 1; x >= 0; x--)
				{
					if(board[x][moveable.at(i).getColumnIndex()].IsEmpty())
					{
						if(!moved  && ValidMove(x, moveable.at(i).getColumnIndex(), 'q', 0, ' ', false))
						{
							if(makemove && decision == movecounter)
							{
								std::cout << "Queen to " << board[x][moveable.at(i).getColumnIndex()].SquareName() << "!" << std::endl;
								moved = true;
					...
					else
					{
						if(board[x][moveable.at(i).getColumnIndex()].getColor() == 1)
								...
								if(makemove && decision == movecounter)
								{
                                                                	std::cout << "Queen takes " << board[x][moveable.at(i).getColumnIndex()].SquareName() << "!" << std::endl;
                                                               	 	moved = true;
									ValidMove(x, moveable.at(i).getColumnIndex(), 'q', 0, ' ', true);
								}
								else if(makemove)
				...
				{	
					if(board[moveable.at(i).getRowIndex()][x].IsEmpty())
					{
						if(!moved  && ValidMove(moveable.at(i).getRowIndex(), x, 'q', 0, ' ', false))
						{
							if(makemove && decision == movecounter)
							{
								std::cout << "Queen to " << board[moveable.at(i).getRowIndex()][x].SquareName() << "!" << std::endl;
								moved = true;
					...
					else
					{
						if(board[moveable.at(i).getRowIndex()][x].getColor() == 1)
								...
								if(makemove && decision == movecounter)
								{
                                                                	std::cout << "Queen takes " << board[moveable.at(i).getRowIndex()][x].SquareName() << "!" << std::endl;
                                                                	moved = true;
									ValidMove(moveable.at(i).getRowIndex(), x, 'q', 0, ' ', true);
								}
								else if(makemove)
				...
				for(int x = 1; x < 8; x++)
								...
								if(makemove && decision == movecounter)
								{
                                                                	std::cout << "Queen to " << board[moveable.at(i).getRowIndex()-x][moveable.at(i).getColumnIndex()-x].SquareName() << "!" << std::endl;
                                                                	moved = true;
                                                ...
                                                else
						{
							if(board[moveable.at(i).getRowIndex()-x][moveable.at(i).getColumnIndex()-x].getColor() == 1)
									...
									if(makemove && decision == movecounter)
									{
                                                                		std::cout << "Queen takes " << board[moveable.at(i).getRowIndex()-x][moveable.at(i).getColumnIndex()-x].SquareName() << "!" << std::endl;
                                                                		moved = true;
										ValidMove(moveable.at(i).getRowIndex()-x, moveable.at(i).getColumnIndex()-x, 'q', 0, ' ', true);
									}
									else if(makemove)
			...
			if(moveable.at(i).getPiece() == 'k')
			{
					...
					if(board[0][5].IsEmpty() && board[0][5].getAttack() == 0 && board[0][6].IsEmpty() && board[0][6].getAttack() == 0 && board[0][7].getPiece() == 'r' && board[0][7].getColor() == 0)
					{
						if(!moved && !check)
						{
							if(makemove && decision == movecounter)
							{
								...
								cancastlekingside = false;
								cancastlequeenside = false;
								std::cout << "I castle! (King's side)" << std::endl;
				...
				if(cancastlequeenside)
				{
					if(board[0][3].IsEmpty() && board[0][3].getAttack() == 0 && board[0][2].IsEmpty() && board[0][2].getAttack() == 0 && board[0][1].IsEmpty() && board[0][0].getPiece() == 'r' && board[0][0].getColor() == 0)
							...
							if(makemove && decision == movecounter)
							{
                                                        	...
                                                        	cancastlekingside = false;
                                                        	cancastlequeenside = false;
                                                        	std::cout << "I castle! (Queen's side)" << std::endl;
					...
					if(board[moveable.at(i).getRowIndex() + 1][moveable.at(i).getColumnIndex()].getColor() != 0 && board[moveable.at(i).getRowIndex()+1][moveable.at(i).getColumnIndex()].getAttack() == 0)
					{
						if(!moved  && ValidMove(moveable.at(i).getRowIndex()+1, moveable.at(i).getColumnIndex(), 'k', 0, ' ', false))
						{
							if(makemove && decision == movecounter)
							{
								...
								{
									std::cout << "King takes " << board[moveable.at(i).getRowIndex()+1][moveable.at(i).getColumnIndex()].SquareName() << "!" << std::endl;
								}
								else
								{
									std::cout << "King to " << board[moveable.at(i).getRowIndex()+1][moveable.at(i).getColumnIndex()].SquareName() << "!" << std::endl;
								}
								moved = true;
				...
				if(moveable.at(i).getColumnIndex() + 1 < 8)
							...
							if(makemove && decision == movecounter)
							{
								...
								{
                                                                	std::cout << "King takes " << board[moveable.at(i).getRowIndex()][moveable.at(i).getColumnIndex()+1].SquareName() << "!" << std::endl;
								}
                                                        	else
								{
                                                   			std::cout << "King to " << board[moveable.at(i).getRowIndex()][moveable.at(i).getColumnIndex()+1].SquareName() << "!" << std::endl;
								}
								moved = true;
				...
				if(moveable.at(i).getColumnIndex() - 1 >= 0 && moveable.at(i).getRowIndex() - 1 >= 0)
							...
							if(makemove && decision == movecounter)
							{
								if(board[moveable.at(i).getRowIndex()-1][moveable.at(i).getColumnIndex()-1].getColor() == 1)
								{
                                                                	std::cout << "King takes " << board[moveable.at(i).getRowIndex()-1][moveable.at(i).getColumnIndex()-1].SquareName() << "!" << std::endl;
								}
                                                        	else
								{
                                                        		std::cout << "King to " << board[moveable.at(i).getRowIndex() - 1][moveable.at(i).getColumnIndex() - 1].SquareName() << "!" << std::endl;
								}
								moved = true;
					...
					if(moveable.at(i).getRowIndex()+x < 8 && moveable.at(i).getColumnIndex()+x < 8)
					{
						if(board[moveable.at(i).getRowIndex()+x][moveable.at(i).getColumnIndex()+x].IsEmpty())
						{
							if(!moved && ValidMove(moveable.at(i).getRowIndex()+x, moveable.at(i).getColumnIndex()+x, 'b', 0, ' ', false))
							{
								if(makemove && decision == movecounter)
								{
									std::cout << "Bishop to "<< board[moveable.at(i).getRowIndex()+x][moveable.at(i).getColumnIndex()+x].SquareName() << "!" << std::endl;
									moved = true;
						...
						else
						{
							if(board[moveable.at(i).getRowIndex()+x][moveable.at(i).getColumnIndex()+x].getColor() == 1)
									...
									if(makemove && decision == movecounter)
									{
                                                                        	std::cout << "Bishop takes " << board[moveable.at(i).getRowIndex()+x][moveable.at(i).getColumnIndex()+x].SquareName() << "!" << std::endl;
                                                                        	moved = true;
										ValidMove(moveable.at(i).getRowIndex()+x, moveable.at(i).getColumnIndex()+x, 'b', 0, ' ', true);
									}
									else if(makemove)
			...
			if(moveable.at(i).getPiece() == 'r')
			{
				 for(int x = 1; x <= (7 - moveable.at(i).getRowIndex()); x++)
							...
							if(makemove && decision == movecounter)
							{
                                                        	std::cout << "Rook to " << board[moveable.at(i).getRowIndex()+x][moveable.at(i).getColumnIndex()].SquareName() << "!"  <<std::endl;
								moved = true;
					...
					else
					{
						if(board[moveable.at(i).getRowIndex()+x][moveable.at(i).getColumnIndex()].getColor() == 1)
									...
									if(makemove && decision == movecounter)
									{
                                                                        	std::cout << "Rook takes " << board[moveable.at(i).getRowIndex()+x][moveable.at(i).getColumnIndex()].SquareName() << "!" << std::endl;
                                                                        	moved = true;
										ValidMove(moveable.at(i).getRowIndex()+x, moveable.at(i).getColumnIndex(), 'r', 0, ' ', true);
									}
									else if(makemove)
						...
						break;
							...
							if(makemove && decision == movecounter)
							{
                                                        	std::cout << "Rook to " << board[moveable.at(i).getRowIndex()][moveable.at(i).getColumnIndex()+x].SquareName() << "!" << std::endl;
								moved = true;
					...
					else
					{
						if(board[moveable.at(i).getRowIndex()][moveable.at(i).getColumnIndex()+x].getColor() == 1)
									...
									if(makemove && decision == movecounter)
									{
                                                                        	std::cout << "Rook takes " << board[moveable.at(i).getRowIndex()][moveable.at(i).getColumnIndex()+x].SquareName() << "!" << std::endl;
                                                                        	moved = true;
										ValidMove(moveable.at(i).getRowIndex(), moveable.at(i).getColumnIndex()+x, 'r', 0, ' ', true);
									}
									else if(makemove)
						...
						break;
							...
							if(makemove && decision == movecounter)
							{
                                                        	std::cout << "Rook to " << board[x][moveable.at(i).getColumnIndex()].SquareName() << "!" << std::endl;
								moved = true;
					...
					else
					{
						if(board[x][moveable.at(i).getColumnIndex()].getColor() == 1)
									...
									if(makemove && decision == movecounter)
									{
                                                                        	std::cout << "Rook takes " << board[x][moveable.at(i).getColumnIndex()].SquareName() << "!" << std::endl;
                                                                        	moved = true;
										ValidMove(x, moveable.at(i).getColumnIndex(), 'r', 0, ' ', true);
									}
									else if(makemove)
						...
						break;
							...
							if(makemove && decision == movecounter)
							{
                                                        	std::cout << "Rook to " << board[moveable.at(i).getRowIndex()][x].SquareName() << "!" << std::endl;
								moved = true;
					...
					else 
					{
						if(board[moveable.at(i).getRowIndex()][x].getColor() == 1)
									...
									if(makemove && decision == movecounter)
									{
                                                                        	std::cout << "Rook takes " << board[moveable.at(i).getRowIndex()][x].SquareName() << "!" << std::endl;
                                                                        	moved = true;
										ValidMove(moveable.at(i).getRowIndex(), x, 'r', 0, ' ', true);
									}
									else if(makemove)
...
int Chess::Decide(double x, std::vector<Move> validmoves)
{
	...
	{
		std::cout << "I don't feel so good..." << std::endl;
	}
	if(bestmove.getScore() > 99999999 && bestmove.getScore() < 999999999)
	{
		std::cout << "i'm feeling pretty good about my next move ngl" << std::endl;
	}
	std::cout << std::endl;
